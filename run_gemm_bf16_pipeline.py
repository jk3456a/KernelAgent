"""Generate and optimize an isolated 4096^3 BF16 GEMM kernel."""
...
DST = REPO / "examples" / "optimize_gemm_bf16"
RUNS_DIR = DST / "runs"
RUN_PREFIX = "gemm_bf16"
...
AGENT2_STRATEGY_ENV = "GEMM_BF16_AGENT2_STRATEGY"

PROBLEM = (
    "Implement a Triton kernel for square matrix multiplication C = A @ B "
    "with M = N = K = 4096. A and B are torch.bfloat16 CUDA tensors with "
    "shapes (4096, 4096), and kernel_function(A, B) must return C with shape "
    "(4096, 4096) and dtype torch.bfloat16. Use a tiled blocked-matmul Triton "
    "kernel with FP32 accumulation internally, then cast once when storing the "
    "BF16 output."
)


def create_run_dir() -> Path:
    """Create an isolated timestamped directory for one BF16 pipeline run."""
